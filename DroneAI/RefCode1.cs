...
namespace SpaceEngineers.UWBlockPrograms.BatteryMonitor
{
    public sealed class Program : MyGridProgram
    {
         ...
         *Added loads of new stuff but then had to delete it because of bugs
         *added new IGC system
        */
        ...
        string VERSION = "Ver_007";                         //Script Current Version
        ...
        void Main(string argument)
        {
            try
            {
                ...
                //This pops the firstitem from the network buffer (if an argument has been recieved)
                if (argument == "IGC_Update")
                { argument = Lstn.AcceptMessage().Data.ToString(); }
                else
                { IGC.SendBroadcastMessage(IGCTagIN, PendingSend); return; }
                //Echo(argument);
                ...
                { MEINFO.LAST_PING = DateTime.Now; MEINFO = DRONE_INFO.SAVE(MEINFO); PendingSend = (MEINFO.OUTPUT); } //RADIO.TransmitMessage(MEINFO.OUTPUT, MyTransmitTarget.Owned); 
                ...
                PendingSend = (MEINFO.OUTPUT + DOCKOUTPUT + DEI_OUT);//  IGC.SendBroadcastMessage((MEINFO.OUTPUT + DOCKOUTPUT + DEI_OUT), IGCTagIN);
                                                                     //RADIO.TransmitMessage(MEINFO.OUTPUT + DOCKOUTPUT + DEI_OUT, MyTransmitTarget.Owned);   OLD ANTENNA COMMS
        ...
        //----------==--------=------------=-----------=---------------=------------=-------==--------=------------=-----------=----------
        #endregion
        //Some fixes because Keen fucking can't literally do anything and decided to break scripts with a new system that's a million times more complicated
        public Program()
        {
            Runtime.UpdateFrequency = UpdateFrequency.Update100;
            //sets itself as a reciever
            Lstn = IGC.RegisterBroadcastListener(IGCTagOUT); //in is drone //out is from central command
